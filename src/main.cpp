...
#if defined(NDEBUG)
# error "Ferncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfe15ce62b7c4e05fcd4bb687a76862de79eec427ca06f40c80577a16a60b9742");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Ferncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Ferncoin Signed Message:\n";
...
{
    // Ferncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Ferncoin: 420k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Ferncoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Ferncoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Ferncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Ferncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0797fdaffad617148533c60db9571b47aaea50e66b008838356f3b8e2383b562");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " 1/3/2022 - Cool Things";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04b47af854a1d62b3e665521cf5799a191891ee1e500f464134fdd8867154edf52041c4d727e9f12a19619978ee44fa9298b113ead25a9e2ea585460f89aea324a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1641229063;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086769489;

        if (fTestNet)
        {
            block.nTime    = 1641229047;
            block.nNonce   = 385633200;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x34c32fdbede2864e9f5ed8dc0eb4d3094f78c701f3de0c208af996a36f203e37"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb2, 0xde }; // Ferncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FerncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FerncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FerncoinMiner(CWallet *pwallet)
{
    printf("FerncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ferncoin-miner");
    ...
    try { loop {
        ...
        printf("Running FerncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FerncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FerncoinMiner, pwallet));
}
